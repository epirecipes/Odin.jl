...
sys <- dust_system_create(sir, true_pars, dt = 1, seed = 42)
...
samples <- monty_sample(posterior, sampler, 500, n_chains = 4,
...
for (sc in scenarios) {
  ...
  for (j in seq_len(n_proj)) {
    ...
    sys <- dust_system_create(sir_proj, pars, dt = 1, seed = j)
    dust_system_set_state_initial(sys)
