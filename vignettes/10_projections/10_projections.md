...
```
    ...
    DustSystemGenerator{var"##OdinModel#277"}(var"##OdinModel#277"(4, [:S, :I, :R, :incidence], [:beta, :gamma, :I0, :N], false, true, false, false, Dict{Symbol, Array}()))
...
obs_result = dust_system_simulate(sir, true_pars; times=times, dt=1.0, seed=42)
...
```
    ...
    MontySamples([0.5971140598811475 0.5971140598811475 … 0.5281914883587675 0.5281914883587675; 0.1683748203883879 0.1683748203883879 … 0.1281312062601164 0.1281312062601164;;; 0.5095697203999469 0.5095697203999469 … 0.5694137007653413 0.5599012486830757; 0.13157265611628058 0.13157265611628058 … 0.11705454330890881 0.10679177252673384;;; 0.5095351240152955 0.5095351240152955 … 0.5027167241163174 0.524557200221153; 0.11623823308600102 0.11623823308600102 … 0.10128108264714918 0.12361302907400067;;; 0.5227624291007619 0.5227624291007619 … 0.5082308291287359 0.5082308291287359; 0.11283506108610089 0.11283506108610089 … 0.10136119854183795 0.10136119854183795], [-108.43610249234388 -107.66504056124494 -106.34928189789619 -105.41658099101292; -108.43610249234388 -107.66504056124494 -106.34928189789619 -105.41658099101292; … ; -106.43351510716263 -108.19813679657051 -106.07662738321075 -104.97733422569233; -106.43351510716263 -107.62755384722723 -106.01618433919623 -104.97733422569233], [0.5 0.5 0.5 0.5; 0.1 0.1 0.1 0.1], ["beta", "gamma"], Dict{Symbol, Any}(:acceptance_rate => [0.582, 0.472, 0.496, 0.502]))
...
```
    ...
    β: mean = 0.518, 95% CI = [0.44, 0.617]
    γ: mean = 0.113, 95% CI = [0.062, 0.195]
    True: β = 0.5, γ = 0.1
...
```
    ...
    DustSystemGenerator{var"##OdinModel#281"}(var"##OdinModel#281"(4, [:S, :I, :R, :incidence], [:beta_time, :beta_value, :gamma, :I0, :N], false, false, false, true, Dict{Symbol, Array}()))
...
for scenario in scenarios
    ...
    for j in 1:n_proj
        ...
        sys = dust_system_create(sir_proj, pars; dt=1.0, seed=j)
...
```
    ...
    Cumulative cases at day 100 (median [95% CI]):
      Baseline: 982 [952, 990]
      Mild intervention (−30%): 982 [952, 990]
      Strong intervention (−60%): 982 [951, 990]

## Summary

| Step | API |
|----|----|
| Fit model | `monty_sample(posterior, sampler, n_steps; n_chains, n_burnin)` |
| Extract posterior | `samples.pars[param, step, chain]` |
| Projection model | `@odin` with `interpolate()` for time-varying β |
| Simulate scenario | `dust_system_create()` + `dust_system_simulate()` per draw |
