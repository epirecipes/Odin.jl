...
sys <- System(sir, pars, n_particles = 1, dt = 1, seed = 1L)
...
``` r
filter <- Likelihood(sir, data = data_df, time_start = 0,
...
packer <- Packer(c("beta", "gamma"), fixed = list(I0 = 10, N = 1000))
ll_model <- as_model(filter, packer)
...
sampler <- random_walk(vcv = diag(c(0.01, 0.001)))
```

Running the full MCMC is commented out for vignette build speed:

``` r
samples <- sample(posterior, sampler, n_steps = 1000, n_chains = 4)
```
