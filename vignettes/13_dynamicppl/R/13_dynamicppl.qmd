...
This vignette shows the R equivalent of the Julia DynamicPPL integration workflow. In R, we use `monty_dsl()` for prior specification and `monty_sample()` for inference, with `odin2`, `dust2`, and `monty`.
...
This is equivalent to the Julia `@monty_prior` and `dppl_prior` approaches shown in the Julia vignette.
...
The R workflow produces equivalent posteriors to the Julia `@monty_prior` and `dppl_prior` approaches. The key differences are:
