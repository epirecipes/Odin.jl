...
banana_no_grad = monty_model(
    banana_density;
    parameters=["x", "y"],
)

banana_with_grad = monty_model(
    banana_density;
    parameters=["x", "y"],
...
``` julia
slice = monty_sampler_slice(w=2.0, max_steps=20)
initial = zeros(Float64, 2, 4)

samples_slice = monty_sample(banana_no_grad, slice, 5000;
...
samples_mala = monty_sample(banana_with_grad, mala, 5000;
    n_chains=4, initial=initial, n_burnin=1000, seed=42)

println("MALA on banana target:")
println("  Mean: ", round.(mean(samples_mala.pars[:, :, :], dims=(2, 3))[:, 1, 1], digits=2))
println("  Acceptance rate: ", round.(samples_mala.details[:acceptance_rate], digits=3))
```
...
``` julia
M = [1.0 0.0; 0.0 4.0]
mala_precond = monty_sampler_mala(0.2; vcv=M)

samples_mala_p = monty_sample(banana_with_grad, mala_precond, 5000;
    n_chains=4, initial=zeros(Float64, 2, 4), n_burnin=1000, seed=42)

println("Preconditioned MALA acceptance: ",
    round.(samples_mala_p.details[:acceptance_rate], digits=3))
```
...
blocks = [[1], [2]]
sub_samplers = [
    monty_sampler_slice(w=2.0),
    monty_sampler_slice(w=2.0),
]
gibbs_slice = monty_sampler_gibbs(blocks, sub_samplers)

samples_gibbs = monty_sample(banana_no_grad, gibbs_slice, 5000;
    n_chains=4, initial=zeros(Float64, 2, 4), n_burnin=1000, seed=42)
...
mixed_subs = Odin.AbstractMontySampler[
    monty_sampler_mala(0.5; vcv=reshape([1.0], 1, 1)),
    monty_sampler_slice(w=2.0),
]
gibbs_mixed = monty_sampler_gibbs(mixed_blocks, mixed_subs)

samples_mixed = monty_sample(banana_with_grad, gibbs_mixed, 5000;
    n_chains=4, initial=zeros(Float64, 2, 4), n_burnin=1000, seed=42)
...
t_rw = @elapsed s_rw = monty_sample(gauss_model, rw, n_steps;
    n_chains=4, initial=initial, n_burnin=n_burnin, seed=42)

# Slice
sl = monty_sampler_slice(w=1.5)
t_sl = @elapsed s_sl = monty_sample(gauss_model, sl, n_steps;
    n_chains=4, initial=initial, n_burnin=n_burnin, seed=42)

# MALA
ml = monty_sampler_mala(0.5)
t_ml = @elapsed s_ml = monty_sample(gauss_model, ml, n_steps;
    n_chains=4, initial=initial, n_burnin=n_burnin, seed=42)

# HMC
hm = monty_sampler_hmc(0.1, 10)
t_hm = @elapsed s_hm = monty_sample(gauss_model, hm, n_steps;
    n_chains=4, initial=initial, n_burnin=n_burnin, seed=42)
...
```
    ...
    RW: ESS(x)=73.0, ESS(y)=74.0, min_ESS/s=321.0, accept=0.323
    Slice: ESS(x)=213.0, ESS(y)=213.0, min_ESS/s=969.0, accept=1.0
    MALA: ESS(x)=58.0, ESS(y)=59.0, min_ESS/s=188.0, accept=0.39
    HMC: ESS(x)=570.0, ESS(y)=571.0, min_ESS/s=589.0, accept=0.985
...
samples_rw <- monty_sample(m, sampler_rw, 5000, n_chains = 4)
