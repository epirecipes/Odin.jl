---
title: "Advanced MCMC Samplers"
subtitle: "Slice, MALA, and Gibbs sampling in Odin.jl"
author: "Odin.jl"
format:
  html:
    code-fold: false
...
Odin.jl provides three advanced MCMC samplers beyond the standard random walk and HMC:
...
A twisted Gaussian ("banana") that challenges samplers with strong nonlinear correlation:
...
The slice sampler uses Neal's stepping-out and shrinking procedure. It requires
no gradient and no tuning of proposal variance — only the bracket width `w`:
...
MALA uses gradient information for smarter proposals. It's simpler than HMC
(one gradient evaluation per step) but still makes directed moves:
...
different sub-sampler — useful for hierarchical models or mixed parameter types.
...
```{julia}
mixed_blocks = [[1], [2]]
mixed_subs = AbstractMontySampler[
