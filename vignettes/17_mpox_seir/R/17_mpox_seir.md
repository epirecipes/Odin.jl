...
sys <- dust_system_create(mpox_seir, true_pars, dt = 0.25, seed = 42)
...
``` r
filter <- dust_filter_create(mpox_seir, time_start = 0,
                             data = filter_data,
...
for (k in seq_len(n_proj)) {
  ...
  proj_sys <- dust_system_create(mpox_seir, pars, dt = 0.25, seed = k)
  dust_system_set_state_initial(proj_sys)
  r <- dust_system_simulate(proj_sys, proj_times)
...
| Simulate        | `dust_system_create()` → `dust_system_simulate()`        |
| Prepare data    | `data.frame(time = …, cases_data = …)`                   |
| Particle filter | `dust_filter_create(gen, time_start, data, n_particles)` |
| Likelihood      | `dust_likelihood_monty(filter, packer)`                  |
| Prior           | `monty_dsl({ … })`                                       |
| Posterior       | `likelihood + prior`                                     |
