...
The benchmarks above are scalar (no array dimensions). We now add a richer
slice: a **3-age-group SEIRD** model with split latent stages, age-specific
susceptibility and case-fatality ratios, and per-age Poisson case/death
observation streams. This exercises the runtime-sized array ODE solver, the
deterministic unfilter for parameter-dependent dimensions, and forward/adjoint
gradients on a 21-state system.
...
All three exercise the deterministic likelihood and gradient pipeline, including
both forward and adjoint modes. The age-structured model additionally validates
the runtime-sized array ODE solver and the dynamic state-cache resizing in the
deterministic unfilter.
