...
sys <- System(seird_regions, true_pars,
...
sys_w <- System(seird_regions, true_pars,
                            ode_control = dust_ode_control())
dust_system_set_state_initial(sys_w)
result_weekly <- simulate(sys_w, obs_weeks)
...
``` r
uf <- Likelihood(seird_regions, time_start = 0, data = data)
...
for (j in seq_len(n_proj)) {
  ...
  sys_j <- System(seird_regions, pars_j,
                              ode_control = dust_ode_control())
  dust_system_set_state_initial(sys_j)
  r <- simulate(sys_j, proj_times)
  for (i in 1:n_regions) {
...
sys_nc <- System(seird_regions, pars_nocoupling,
                             ode_control = dust_ode_control())
dust_system_set_state_initial(sys_nc)
result_nc <- simulate(sys_nc, sim_times)
...
| Pack parameters | `Packer(c("rho", "ifr"), array = list(Rt_v1 = 8, …), fixed = …)` |
