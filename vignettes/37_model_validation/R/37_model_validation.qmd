---
title: "Model Validation and Diagnostics (R)"
format:
  html: default
  gfm: default
---
...
After fitting a model to data, we need to assess whether the fit is adequate. This vignette demonstrates model validation in R using:
...
We use the `bayesplot` package for visualisation and base R for diagnostics, applied to an SIR model fitted to synthetic data.
...
For this vignette, we simulate "posterior samples" by adding noise around the true parameters (in practice, use MCMC via `monty`):
...
A key sanity check: the posterior predictive should be narrower than the prior predictive:
...
The `bayesplot` package (Gabry, Simpson, Vehtari, Betancourt, Gelman, 2019) provides the most comprehensive R visualisation for posterior predictive checks.
