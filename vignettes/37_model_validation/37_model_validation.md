...
```
    ...
    DustSystemGenerator{var"##OdinModel#277"}(var"##OdinModel#277"(3, [:S, :I, :R], [:beta, :gamma, :I0, :N], true, false, false, true, false, Dict{Symbol, Array}()))
...
result = dust_system_simulate(sir, true_pars; times=times, dt=0.25)

# Extract the cases output variable
sys = dust_system_create(sir, true_pars; n_particles=1, dt=0.25)
...
``` julia
pp = posterior_predictive(samples, sir;
...
wrong_samples = MontySamples(wrong_pars_arr, wrong_density, wrong_pars_arr[:, 1, :],
                              ["beta", "gamma"], Dict{Symbol, Any}())

wrong_pp = posterior_predictive(wrong_samples, sir;
    times=times, n_draws=100, dt=0.25,
    output_vars=[:cases],
    packer=packer, seed=42)
```
