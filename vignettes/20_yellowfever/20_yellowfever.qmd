---
title: "Yellow Fever SEIRV: Age-Structured Model with Spillover"
subtitle: "Odin.jl Vignette 20"
format:
  html:
    toc: true
    code-fold: false
...
Yellow fever is a mosquito-borne flavivirus endemic in tropical Africa and
South America. In addition to human-to-human (urban) transmission, **sylvatic
spillover** from non-human primate reservoirs drives outbreaks even in
partially vaccinated populations. The YF-17D vaccine provides long-lasting
immunity but coverage varies by age group and year.

This vignette builds an **age-structured discrete-time stochastic SEIRV
model** inspired by the [YEP](https://github.com/mrc-ide/YEP) package. Key
features:
...
across compartments. The youngest age group receives external inflow
via `dP1[1]` added to susceptibles.
...
population sizes reflecting a typical sub-Saharan African age distribution:
...
1. **Partial array updates** (`update(S[1])` vs `update(S[2:N_age])`) allow
   different aging rules for the youngest vs older age groups — inflow from
   births for group 1, vs aging from the previous group for groups 2+.

2. **Interpolated time-varying parameters** (`R0_t`, `FOI_sp`) provide
   flexible seasonal and episodic forcing without requiring the model to
   track time indices explicitly.

3. **`max(0, ...)`** floors prevent negative populations from stochastic
   overshooting, a common issue in discrete-time models with large time steps
   or small compartments.

4. **Population dynamics** (demographic aging flows) can create small
   conservation errors when combined with the max-floor, but the relative
   change remains negligible over the simulation period.
