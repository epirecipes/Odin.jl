...
## Solver selection in `simulate`

When using the odin DSL with `simulate`, you can select the solver via
the `solver` keyword argument:
...
output = simulate(sys, times; solver=:sdirk)
```

## Solver selection in `Likelihood`

For likelihood computation with `Likelihood`, pass the solver through
`ODEControl`:

``` julia
# Default (DP5)
ctrl = ODEControl(atol=1e-8, rtol=1e-8)

# Stiff solver
ctrl = ODEControl(atol=1e-8, rtol=1e-8, solver=:sdirk)
unfilter = Likelihood(generator, data; ode_control=ctrl)
```
...
- Fully integrated with `simulate` and `Likelihood`
