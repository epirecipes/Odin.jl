...
```
    ...
    Odin.DustSystemGenerator{var"##OdinModel#277"}(var"##OdinModel#277"(3, [:S, :I, :R], [:beta, :gamma, :sigma, :I0, :N], true, true, false, false, false, Dict{Symbol, Array}()))
...
SDE models use fixed time-stepping. The `dt` parameter in `System` sets
the integration step size (smaller = more accurate but slower):

``` julia
pars = (beta=0.5, gamma=0.1, sigma=0.1, I0=10.0, N=1000.0)
sys = System(sir_sde, pars; dt=0.01, seed=42)
reset!(sys)
...
sys_ode = System(sir_ode, pars_ode)
reset!(sys_ode)
...
``` julia
n_particles = 100
sys_ensemble = System(sir_sde, pars; n_particles=n_particles, dt=0.01, seed=123)
reset!(sys_ensemble)
ensemble = simulate(sys_ensemble, times)
...
``` julia
# Euler-Maruyama
sys_em = System(sir_sde, pars; dt=0.01, seed=42)
reset!(sys_em)
out_em = simulate(sys_em, times; solver=:euler_maruyama)

# Milstein
sys_mil = System(sir_sde, pars; dt=0.01, seed=42)
reset!(sys_mil)
...
for σ_val in [0.01, 0.1, 0.5]
    ...
    s = System(sir_sde, p; n_particles=50, dt=0.01, seed=42)
    reset!(s)
    out = simulate(s, times)
