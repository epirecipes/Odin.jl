...
```
    ...
    Odin.DustSystemGenerator{var"##OdinModel#277"}(var"##OdinModel#277"(3, [:S, :I, :R], [:beta, :gamma, :I0, :N], true, false, true, false, false, Dict{Symbol, Array}()))
...
```
    ...
    Odin.DustSystemGenerator{var"##OdinModel#278"}(var"##OdinModel#278"(4, [:S, :E, :I, :R], [:beta, :gamma, :sigma, :I0, :N], true, false, true, false, false, Dict{Symbol, Array}()))
...
sim = simulate(sir, true_pars; times=[0.0; times], dt=0.25)
...
ll_sir = loglik(uf_sir, pars_sir)

# SEIR unfilter
uf_seir = Likelihood(seir, data; time_start=0.0)
pars_seir = (beta=0.8, gamma=0.1, sigma=0.2, I0=10.0, N=1000.0)
ll_seir = loglik(uf_seir, pars_seir)
...
aic_sir = aic(ll_sir, k_sir)
aic_seir = aic(ll_seir, k_seir)

bic_sir = bic(ll_sir, k_sir, n_obs)
bic_seir = bic(ll_seir, k_seir, n_obs)
...
for s in 1:n_posterior
    ...
    pw_sir[:, s] = loglik_pointwise(uf_sir, p)
end
...
for s in 1:n_posterior
    ...
    pw_seir[:, s] = loglik_pointwise(uf_seir, p)
end

waic_sir = waic(pw_sir)
waic_seir = waic(pw_seir)
