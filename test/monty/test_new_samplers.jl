...
@testset "New Samplers" begin
    ...
    @testset "Slice sampler — basic convergence" begin
        sampler = slice(w=1.0, max_steps=10)
    ...
    @testset "Slice sampler — no gradient required" begin
        ...
        sampler = slice(w=2.0, max_steps=20)
        initial = zeros(Float64, 2, 1)

        samples = sample(model_no_grad, sampler, 1000;
    ...
    @testset "Slice sampler — parameter validation" begin
        @test_throws ErrorException slice(w=-1.0)
        @test_throws ErrorException slice(w=1.0, max_steps=0)
    end

    @testset "Slice sampler — respects domain" begin
        # Positive-only domain
        pos_model = DensityModel(
        ...
        samples = sample(pos_model, sampler, 2000;
    ...
    @testset "Slice sampler — variance recovery" begin
        sampler = slice(w=2.0, max_steps=15)
        initial = zeros(Float64, 2, 4)

        samples = sample(model_no_grad, sampler, 5000;
    ...
    @testset "MALA — basic convergence" begin
        ...
        samples = sample(model_with_grad, sampler, 3000;
            n_chains=2, initial=initial, n_burnin=1000, seed=42)
    ...
    @testset "MALA — requires gradient" begin
        ...
        @test_throws ErrorException sample(model_no_grad, sampler, 100;
            n_chains=1, initial=initial, seed=42)
    end

    @testset "MALA — parameter validation" begin
        @test_throws ErrorException mala(-0.1)
    end

    @testset "MALA — with mass matrix" begin
        M = [1.0 0.0; 0.0 2.0]
        sampler = mala(0.2; vcv=M)
        initial = zeros(Float64, 2, 2)

        samples = sample(model_with_grad, sampler, 2000;
            n_chains=2, initial=initial, n_burnin=500, seed=42)
    ...
    @testset "MALA — converges faster than RW" begin
        ...
        rw_sampler = random_walk(Matrix{Float64}(0.3I, 2, 2))
        mala_sampler = mala(0.3)
        initial = zeros(Float64, 2, 2)
        n_steps = 2000

        rw_samples = sample(model_with_grad, rw_sampler, n_steps;
            n_chains=2, initial=initial, n_burnin=500, seed=42)
        mala_samples = sample(model_with_grad, mala_sampler, n_steps;
            n_chains=2, initial=initial, n_burnin=500, seed=42)
    ...
    @testset "Gibbs — parameter validation" begin
        @test_throws ErrorException gibbs(Vector{Int}[], Odin.AbstractMontySampler[])
        @test_throws ErrorException gibbs(
            [[1], [2]],
    ...
    @testset "ESS comparison across samplers" begin
        ...
        # Slice
        sl = slice(w=1.5)
        s_slice = sample(model_no_grad, sl, n_steps;
            n_chains=2, initial=initial, n_burnin=n_burnin, seed=42)

        # MALA
        ml = mala(0.3)
        s_mala = sample(model_with_grad, ml, n_steps;
            n_chains=2, initial=initial, n_burnin=n_burnin, seed=42)

        # RW (baseline)
        rw = random_walk(Matrix{Float64}(0.5I, 2, 2))
        s_rw = sample(model_no_grad, rw, n_steps;
            n_chains=2, initial=initial, n_burnin=n_burnin, seed=42)
